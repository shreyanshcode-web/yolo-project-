        ...
        with torch.no_grad():
            ...
            while True:
                ...
                if frame is not None:
                    ...
                    if output_path is not None:
                        # Ensure subfolders exist for depth and rgb outputs
                        output_path_depth = os.path.join(output_path, 'depth')
                        output_path_rgb = os.path.join(output_path, 'rgb')
                        os.makedirs(output_path_depth, exist_ok=True)
                        os.makedirs(output_path_rgb, exist_ok=True)
                        ...
                        # read back the depth map (grayscale) and compute farthest point
                        depth_map = cv2.imread(filename_depth + ".png", cv2.IMREAD_GRAYSCALE)
                        if depth_map is not None:
                            try:
                                point = farthest_point(depth_map)
                            except NameError:
                                # If farthest_point is not defined, fallback to finding min pixel location
                                idx = np.unravel_index(np.argmin(depth_map), depth_map.shape)
                                point = (int(idx[0]), int(idx[1]))
                            # write to txt file
                            with open(os.path.join(output_path_depth, 'point.txt'), 'w') as f:
